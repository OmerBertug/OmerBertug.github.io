...
import {
  ...
  monotonicDayIndexToIsoDate,
  isWideningAppLimit,
  MS_PER_DAY,
} from '../src/invariants.js';
import type { TimeWindow, ProtectionState, ShortVideoConfig, PlatformUsage, AppEntry, AppGroup } from '@aegis/schemas';

// ─── Helpers ──────────────────────────────────────────────────────────────────

const makeWindow = (start: number, end: number): TimeWindow => ({
...
const makeUsage = (partial: Partial<PlatformUsage>): PlatformUsage => ({
  INSTAGRAM_REELS: 0,
  YOUTUBE_SHORTS: 0,
  TIKTOK_EXPLORE: 0,
  REDDIT_VIDEOS: 0,
