...
const baseProtection: ProtectionState = {
  immutableMode: false,
...
const baseContext = { minuteOfDay: 600, confidence: 'high' as const, trustedEpochMs: 1700000000000 };
const lockedContext = { minuteOfDay: 600, confidence: 'high' as const, trustedEpochMs: 1700000000000 };
const lowConfContext = { minuteOfDay: 1435, confidence: 'low' as const, trustedEpochMs: 1700000000000 };
...
const baseAppBlockingState: AppBlockingState = {
...
const baseWebState: WebsiteBlockerState = {
  nsfwFilterEnabled: false,
  customBlacklist: [],
  vpnActive: false,
};

const baseNotifState: NotificationsState = {
  silenced: {},
  retention: 'P1M',
};
...
describe('setImmutableMode', () => {
  it('succeeds when immutableMode is off', () => {
    ...
    if (isOk(result)) expect(result.value.immutableMode).toBe(true);
  });

  it('T7 — low confidence → E_LOCKED', () => {
    const result = setImmutableMode(baseProtection, false, lowConfContext);
    expect(isErr(result)).toBe(true);
    if (isErr(result)) expect(result.error.code).toBe('E_LOCKED');
  });
  ...
  it('updates lastMutationAt to trustedEpochMs', () => {
    const result = setImmutableMode(baseProtection, true, baseContext);
    expect(isOk(result)).toBe(true);
    if (isOk(result)) expect(result.value.lastMutationAt).toBe(baseContext.trustedEpochMs);
  });
});
...
describe('setImmutableWindow', () => {
  it('rejects invalid window (not 10 min)', () => {
    const result = setImmutableWindow(
      baseProtection,
      { startMinuteOfDay: 600, endMinuteOfDay: 620 }, // 20 min = invalid
      baseContext,
    );
    expect(isErr(result)).toBe(true);
    if (isErr(result)) expect(result.error.code).toBe('E_INVALID_WINDOW');
  });
...
describe('setAppDailyLimit', () => {
  ...
  it('unknown appId → E_INVALID_SCHEMA', () => {
    ...
    expect(isErr(result)).toBe(true);
    if (isErr(result)) expect(result.error.code).toBe('E_INVALID_SCHEMA');
  });
...
describe('addWebsiteToBlacklist', () => {
  it('valid hostname succeeds', () => {
    ...
    expect(isOk(result)).toBe(true);
    if (isOk(result)) expect(result.value.customBlacklist).toContain('example.com');
  });
  ...
  it('reserved TLD "localhost" → E_INVALID_SCHEMA', () => {
    ...
    expect(isErr(result)).toBe(true);
    if (isErr(result)) expect(result.error.message).toContain('ayrılmış TLD');
  });
  ...
  it('duplicate hostname → E_DUPLICATE_HOST', () => {
    const stateWithEntry: WebsiteBlockerState = {
      ...baseWebState,
      customBlacklist: ['example.com'],
    };
    const result = addWebsiteToBlacklist(stateWithEntry, 'example.com');
    expect(isErr(result)).toBe(true);
    if (isErr(result)) expect(result.error.code).toBe('E_DUPLICATE_HOST');
  });
...
describe('removeWebsiteFromBlacklist', () => {
  ...
  it('removing when unlocked → success', () => {
    ...
    expect(isOk(result)).toBe(true);
    if (isOk(result)) expect(result.value.customBlacklist).toHaveLength(0);
  });
...
describe('setNsfwFilter', () => {
  it('T15 — disabling NSFW while locked → E_LOCKED', () => {
    const state: WebsiteBlockerState = { ...baseWebState, nsfwFilterEnabled: true };
...
describe('setSilenced', () => {
  it('silences an app', () => {
    ...
    expect(isOk(result)).toBe(true);
    if (isOk(result)) expect(result.value.silenced['com.instagram.android']).toBe(true);
  });
  ...
  it('unsilences an app', () => {
    ...
    expect(isOk(result)).toBe(true);
    if (isOk(result)) expect(result.value.silenced['com.instagram.android']).toBe(false);
  });
});
...
describe('setRetention', () => {
  it('sets all valid retention values', () => {
    const retentions = ['P15D', 'P1M', 'P3M', 'P6M', 'P1Y'] as const;
    for (const r of retentions) {
      const result = setRetention(baseNotifState, r);
      expect(isOk(result)).toBe(true);
      if (isOk(result)) expect(result.value.retention).toBe(r);
    }
  });
});
